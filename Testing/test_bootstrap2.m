...
omega = (3*pi)/8; % used for the simulations, in the models below, causality from 1<-2 will NOT
...
for i = 1:nsim
   ...
   nboot = 100;
   tic;
   boot(i) = bootstrap_mvar_spectral(v,n,pmin,pmax,nfft,fs,nboot,mvar_point(i),'granger',{l m});
   toc
end
...
if nsim > 1
   ...
   plot(f,sum(cat(2,bc.pval_yz)<.05,2)./nsim,'r');
   temp = sum(cat(2,boot.pval_yz)<.05,2)./nsim;
   plot(f,temp(1:length(f)),'k')   
   axis([0 pi 0 1]);

   subplot(2,2,2);hold on
   plot(f,sum(cat(2,bc.pval_zy)<.05,2)./nsim,'r--');
   temp = sum(cat(2,boot.pval_zy)<.05,2)./nsim;
   plot(f,temp(1:length(f)),'k--')   
   axis([0 pi 0 1]);
end